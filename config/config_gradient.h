#ifndef CONFIG_GRADIENT_H
#define CONFIG_GRADIENT_H
...
namespace Gradient
{
    ...
    Color Multiply(double k, const Color &color)
    {
        return Color(
            static_cast<int>(std::clamp(k * color.r, 0.0, 255.0)),
            static_cast<int>(std::clamp(k * color.g, 0.0, 255.0)),
            static_cast<int>(std::clamp(k * color.b, 0.0, 255.0)));
    }

    // DrawShadedTriangle implementation
    inline void DrawShadedTriangle(Point p0, Point p1, Point p2, const Color &color)
    {
        // Sort the points from bottom to top
        if (p1.y < p0.y)
            std::swap(p0, p1);
        ...
        {
            int idx = y - p0.y;
            int xl = static_cast<int>(std::round(x_left[idx]));
            int xr = static_cast<int>(std::round(x_right[idx]));
            auto h_segment = Interpolate(xl, h_left[idx], xr, h_right[idx]);
            for (int x = xl; x <= xr; ++x)
            {
                PutPixel(x, y, color * h_segment[x - xl]);
            }
        }
    }
